...
async function pollSingleTask(task: any, db: any, maxRetries: number, now: number) {
  ...
  try {
    ...
    if (mappedStatus === 'completed' || mappedStatus === 'succeeded') {
      updateData.resultUrl = statusData.resultUrl || statusData.output?.video_url || statusData.output?.image_url
      updateData.resultData = JSON.stringify(statusData)
      updateData.completedAt = now
      updateData.progress = 100
      if (task.apiTokenId && updateData.creditsUsed) {
        await incrementUsage(task.apiTokenId, Math.ceil(updateData.creditsUsed))
      }
...
async function incrementUsage(tokenId: number, amount: number) {
  const db = getDb()
